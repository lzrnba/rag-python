...
| Cross-Encoder 重排            | ⏳ 未完成 | 仍为预留能力（注释标注暂未实现）                      |
...
- 项目已稳定越过 **Phase 1**。
- 当前处于 **Phase 2 收尾阶段**（核心能力已落地，仅剩重排与术语增强）。
...
**Phase 1 核心功能已完全实现**，系统包含完整的 LangGraph 状态图、4 个协作的 Agent 节点、混合检索实现、FastAPI 服务框架以及完善的文档和测试。
...
**下一步计划**：
1. 集成真实 FAISS 向量库（bge-m3 Embedding）
2. 引入 jieba 分词优化中文 BM25
3. 支持 PDF/Word 文档解析
4. 添加 Cross-Encoder 重排
5. 实现多轮对话记忆
